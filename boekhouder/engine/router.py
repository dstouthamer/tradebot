...
from boekhouder.agents.cfo import CfoAgent, Financials
...
class Router:
    def __init__(self) -> None:
        ...
        self.profile = CompanyProfile.from_settings(self.settings)
        ...
        self.invoice = InvoiceAgent(self.profile)
        self.quote = QuoteAgent(self.profile)
    ...
    def handle(self, message: str, *, session_id: str = "default",
        ...
        return handler(intent, session_id=session_id,
                       image_bytes=image_bytes, file_content=file_content)

    # ------------------------------------------------------------------ #
    def _confirm(self, intent: Intent, *, session_id: str, **_) -> Reply:
        outcome = self.gate.confirm(session_id)
        zone = RiskZone.GROEN if outcome["ok"] else RiskZone.ORANJE
        return Reply(outcome["message"], "audit", zone)

    def _invoice(self, intent: Intent, *, session_id: str, **_) -> Reply:
        res = self.invoice.run(intent)
        ...
        bk = get_bookkeeping()
        self.gate.propose(
            PendingAction(session_id, "factuur", res.summary, inv,
        ...
        text = formatters.factuur(inv, self.profile.name, self.profile.iban) + \
            f"\n\n{res.advies}"
        return Reply(text, "invoice", res.risk_zone, requires_confirmation=True)

    def _quote(self, intent: Intent, *, session_id: str, **_) -> Reply:
        res = self.quote.run(intent)
        if res.payload is None:
            return Reply(f"{res.summary} Nog nodig: {', '.join(res.bewijs_nodig)}.",
                         "quote", res.risk_zone)
        q = res.payload
        self.gate.propose(
            PendingAction(session_id, "offerte", res.summary, q),
            actor="gebruiker", confidence=res.confidence, proposed_action="maak_offerte")
        text = formatters.offerte(q, self.profile.name) + f"\n\n{res.advies}"
        return Reply(text, "quote", res.risk_zone, requires_confirmation=True)

    def _bon(self, intent: Intent, *, session_id: str, image_bytes=None, **_) -> Reply:
        ...
        if res.risk_zone == RiskZone.ROOD:
            self.store.add_controlelijst("bon", res.summary)
            return Reply(f"{res.summary} Nodig: {', '.join(res.bewijs_nodig)}.",
                         "ocr", res.risk_zone)
        doc = res.payload
        match_res = self.matcher.run(doc, self.store.get_bank_txns())
        ...
        if book_res.risk_zone != RiskZone.GROEN:
            self.store.add_controlelijst("boeking", book_res.summary)
        self.gate.propose(
            PendingAction(session_id, "boeking", book_res.summary, boeking),
    ...
    def _bank(self, intent: Intent, *, file_content=None, **_) -> Reply:
        if not file_content:
        ...
        self.store.save_bank_txns(txns)
        ...
        return Reply(
            ...
            f"Stuur nu bonnen om te koppelen.", "bank", RiskZone.GROEN)
    ...
    def _cfo(self, intent: Intent, **_) -> Reply:
        res = self.cfo.run(self._financials())
    ...
    def _financials(self) -> Financials | None:
        # MVP: no live aggregation yet; CFO will ask for data when empty.
        return None
